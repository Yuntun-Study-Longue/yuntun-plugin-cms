...
class BaseInfoAdd extends Component {
    ...
    //         // console.log('===============', values)
    //         // axios.post(`/sysware/api/org-udef/save`, values, { headers: { 'Authorization': Cookies.get('Authorization')}}).then(res => {
    //         //     if (res.status==200) {
    ...
    // }
    saveAndAdd(){
        this.props.form.resetFields(['attrCode','attrType','describe','orgExtends'])
    }
    render() {
        ...
        const orgExtendsProps =getFieldProps('orgExtends', {
            initialValue: this.props.baseData.orgExtends,
            rules: [{type: 'array'}]
        })
        return <Form inline>
            ...
            {
                (this.state.attrType === 1 || this.state.attrType === 5) && 
                (
                    ...
                    <FormItem label='枚举值：' {...fourColLayout}>
                        <ModalTable 
                            listOption={{
                                ...
                                queryBuilder: (item) => item ? ``: `?attrid=${this.props.baseData.id}`
                            }}
                            addOption={{
                            ...
                            controlIcons={['add', 'edit', 'del', 'moveup', 'movedown']} {...orgExtendsProps}/>
                    </FormItem>
                )
            }
